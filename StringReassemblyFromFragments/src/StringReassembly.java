...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        String combination = str1.substring(0, lengthStr1 - overlap) + str2;
        return combination;
    }
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        // check the precondition that strSet contains no substring pairs
        ...
        // executes the following code when the precondition is met
        if (!subString) {
            boolean contain = false;

            // check whether the string is substring of string from strSet
            // if yes, change the boolean contain to true;
            for (String x : strSet) {
                if (x.contains(str) && !contain) {
                    contain = true;
                }
            }

            // if the string is not substring of string from strSet
            // add the string to the set
            // if it is added, also removes from any string already in strset
            // that is a substring of str
            if (!contain) {
                strSet.add(str);

                Set<String> setCopy = strSet.newInstance();
                setCopy.transferFrom(strSet);
                while (setCopy.size() > 0) {
                    String x = setCopy.removeAny();
                    // check whether str itself is being checked
                    // if no, check whether there's any string already in strset
                    // that is a substring of str
                    // if no, add the string back to the strset
                    if (!str.equals(x)) {
                        if (!str.contains(x)) {
                            strSet.add(x);
                        }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        Set<String> inputLines = new Set1L<>();

        // Reports whether the end of the stream has been reached.
        while (!input.atEOS()) {
            String lines = input.nextLine();
            // add the lines to the string if it is not substring of string
            // which is already in the subset
            addToSetAvoidingSubstrings(inputLines, lines);
        }
        ...
        if (!subString) {
            ...
            while ((strSet.size() > 1) && !done) {
                ...
                if (bestOverlap == 0) {
                     ...
                     * SetAvoidingSubstrings because the latter would do the
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        // if char is "~", replace it with a line separator
